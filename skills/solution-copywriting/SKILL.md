...
- [ ] 步骤的硬性前提（驱动版本、端口占用、磁盘余量）写在 `### 前置条件` / `### Prerequisites` 里，而不是混在正文段落？
- [ ] 中英文步骤 ID（`{#...}` 部分）完全一致？
...
| 英文标题 | 中文标题 | 用途 | 渲染位置 |
|---------|---------|------|---------|
| `### Prerequisites` | `### 前置条件` | 该步骤的硬性前提（驱动、端口、磁盘） | 步骤正文上方（标题「开始之前」） |
| `### Wiring` | `### 接线` | 接线说明 | 部署按钮上方（图片 + 列表项渲染成编号步骤；列表之外的散文渲染在列表下方） |
