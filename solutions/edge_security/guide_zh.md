...
**工作台 → 视频墙。** 自适应网格——1、2、4、6、9 格，或者「自动」，自动选能装下当前
全部流的最小布局。每格叠检测器给出的框（带 track id 和分数），以及该路自己的禁区和
越线，所以你看的是「规则会拿什么去判」，而不只是「画面里有没有人」。

网格上方那条是先看的：几路在线 / 共几路、几路离线、几路回落到软解码、合计帧率。

**按 `F` 进全屏**，`Esc` 退出——这就是控制室接 HDMI 大屏的模式，顶栏消失，画面铺满。

设备离线、或者流处在重连状态的格子会转灰并写明原因，底图回落到 hub 缓存的最近一帧。
它不会给一台已经不出图的摄像头继续画实时画面。

#### 不重启就改置信度

1、2、4 格布局下每格带一个置信度滑块。拖动它，检测器从下一帧起就用新阈值——不用重启，
不用重新部署。这个值同时写回 `config/detector.yaml`，所以重启后也还在。

调低更灵敏、误报更多；调高更保守。低到 0.25 以下时，int8 模型多出来的是低置信度的框，
不是原本漏掉的人。

每次改动都有记录：`GET /api/audit` 列出谁改的、从多少改到多少、设备收没收。**如果界面
说结果未知，意思是检测器没有回应**——去设备页刷新，读检测器自己上报的值，不要替它断言
成功或失败。

#### 在浏览器里加一路摄像头

**视频墙 → 添加摄像头。** 选挂到哪台检测器，粘贴 RTSP 地址，起个名字，流 ID 会自动填
下一个可用的。按钮会停在「正在连接摄像头…」直到检测器真的把流打开，所以返回成功就意味
着这一格已经在出图，而不是排进了队列。地址里的密码在下方回显时是打码的。

一个检测器进程可以带多路摄像头：它们共用加速器和模型会话，各自有独立的采集循环和跟踪
器。加一路，去设备页看检测器的 CPU 数字，还有余量再加下一路。格子上的垃圾桶图标可以摘
掉一路摄像头，它此前产生的告警不受影响。
...
一个容器，各自配一套 `device_id`、`stream_id` 和 `preview_port`，也可以用 RK3588
套餐上一块独立板卡。

在 Orin NX 16GB 上，多进程这条路实测可以撑到八路：每多一个检测器进程占 208 MB 统一
内存，八路 1080p 15 fps 是 120 次推理/秒，对实测 236 次/秒的 GPU 上限约 51%。四个
进程各以 5 fps 推理时互不干扰（p50 4.02–4.07 ms，与单进程独跑一致）。正式按八路
摄像头交付之前，请在自己的目标分辨率上实测两项：1080p 15 fps 下每路的 CPU 占用，
以及 NVDEC 的并发会话容量。
