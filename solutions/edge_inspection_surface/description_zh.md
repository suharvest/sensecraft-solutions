...
crazing 的 AP50 是 0.3603，六类中最低，0.35 阈值下召回 0.3969。改阈值不改变
这两个数字。以 crazing 为主要缺陷的产线需要重训模型。
...
**crazing 换架构后 AP50 不变。** 三者都在 0.30-0.36（YOLOX 0.360、
D-FINE 0.302、RT-DETRv2 0.310），与 Jetson 实测边界表里 YOLOX 的数字一致。
