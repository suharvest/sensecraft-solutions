...
| 设备上构建 TensorRT engine，全新部署交叉验证 | 304 s | 同一台设备、同一份 ONNX，删除旧 engine 后从全新部署重新构建；构建脚本必须带 `TRT_STATIC_SHAPE=true`，否则 trtexec 报「Static model does not take explicit shapes」；比上面 291 s 高 4.5%，属正常波动 | 本次实测，「2026-09-08-orin-nx-acceptance」 |
