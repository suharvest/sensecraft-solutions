...
p95 3.249 ms（纯推理）。这份容器的从零部署也在同一台真机上单独做过端到端
验证——`/healthz`、`/trigger` 与 MQTT 输出都对着同一份 val 集的 1060 张
子集核对过（一致率 0.9425、对真值准确率 0.8453、p50 3.167 ms）。
