...
0.8889，RK3588 的 INT8 构建与 fp32 参照的一致率为 0.9893。

### 平台支持

| 平台 | 状态 |
|---|---|
| Jetson Orin（TensorRT） | 已在 reComputer J40 系列（Orin NX）上完成部署与 engine 构建 |
| reComputer R2000（Hailo-8） | 已有部署包；基线 HEF 已在真实 Hailo-8 上跑完全量验证集 |
| RK3588 | fp16 与 INT8 的推理一致性已在真机实测；**没有部署包**——转换与运行时都跑通了，缺的是打包 |
| RK3576 | 推理一致性已在真机实测，但只覆盖已淘汰的骨干；没有部署包 |
