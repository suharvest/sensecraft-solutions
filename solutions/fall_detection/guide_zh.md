...
摄像头本身承载不了面板。那里的检测器是以 .deb 安装的原生进程，摄像头也没有可供面板使用的文件
...
摄像头本身承载不了面板。那里的检测器是应用中心里的一个应用，摄像头也没有可供面板使用的文件
系统、SQLite 与 Web 服务。所以面板装在同网段的另一台机器上，这台机器不需要 AI
算力：reComputer R1000 系列，或者你已有的一台 Linux 机器。
...
5. 2026-09-05 的生产链路测试先停止其他推理应用，统一使用 640×640 H.264、15 FPS RTSP 片源。YOLOv8s INT8 最佳性能配置在每路至少 14.5 FPS 的通过线下验证 RK3576 1 路、RK3588 5 路，两个设备的下一路数边界都失败。当前部署表单仍安装单路，并保留原有 YOLO11n FP16 模型与板卡专属时序权重。
