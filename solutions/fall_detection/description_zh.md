...
**测量条件**：两块 Jetson 的数字与上游口径一致，是在板卡其他业务共存时测得的（上游同样
标注为「共存容量，不冒充空闲峰值」）。停掉共存业务后在 Orin NX 上复测，FP16 为
3.3 ms / 306 FPS，约有 15% 余量；表中保留共存数字，因为那更接近实际部署环境。

**Jetson 为什么不在 INT8 表里**：上游没有 INT8 标定流程，`build_engine.sh` 只传
`--fp16`，要做 INT8 得先在项目里实现标定器和标定集，不是加个开关。用未标定的引擎在空闲
的 Orin NX 上实测过速度上限（2.45 ms / 408 FPS，相对 FP16 快 1.33 倍，与 RK 的 1.5–1.7
倍同向），但未标定的 INT8 动态范围是随意取的，只能看速度、检测结果不可用，因此不作为
可部署配置列入表中。
