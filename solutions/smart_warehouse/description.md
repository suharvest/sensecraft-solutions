...
- **Concurrent stock-in on the same material fails with HTTP 409 — fixed in `fix/a2-concurrency`, pending re-test on harvest-pi.** The numbers in the table above (77.4% at concurrency 5, 100% at 10 and above) come from the pre-fix image. Cause: the generated `batch_no` came from "read today's highest sequence, add one", so concurrent requests read the same committed state, computed the same number, and the fixed 5-attempt retry could not escape it. The fix allocates the number from an atomic counter table (`batch_no_sequences`); the `YYYYMMDD-NNN` format and existing data are unchanged. **Until the re-test confirms it, treat this as unfixed: serialize stock-in per material and retry a 409 client-side with backoff.**
- **Stock-out is rate limited to 60 requests per minute per source IP — fixed in `fix/a2-concurrency`, pending re-test on harvest-pi.** The 95–98% 429 rate above comes from the pre-fix image. The fix keys the limit on the authenticated caller (API key / session) and falls back to the source IP only when no identity is present; the threshold comes from `BUSINESS_RATE_LIMIT`, default 600/minute (10 req/s). The anti-enumeration limits on registration, password recovery and device verification stay keyed on IP. **Until the re-test confirms it, treat this as unfixed: keep sustained stock-out below 1 request/s per exit IP.**
