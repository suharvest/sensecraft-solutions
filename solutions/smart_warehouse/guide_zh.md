...
| 压力下入库返回 409 | 原因：并发对同一物料入库时，生成唯一批次号的固定 5 次重试预算被耗尽（`backend/app.py` 约 4611 行）。规避：按物料串行入库（同一时刻只发一个请求），客户端收到 409 后带退避重试。Raspberry Pi 5 实测：并发 1 错误率 0%，并发 5 为 77.4%，并发 10 及以上为 100% |
| 出库返回 429 | 原因：`slowapi` 对 `/api/materials/stock-out` 按来源 IP 限流 60 次/分钟，与并发数无关。规避：单个出口 IP 的持续出库速率控制在 1 次/秒以下；同一 NAT 后面的终端共用这一份 60 次/分钟额度，繁忙点位需要单独的出口 IP 或错峰发送 |
| 网络或服务中断期间请求丢失 | 原因：REST 层没有离线队列和写入缓冲——重连与退避只覆盖 MCP 语音 WebSocket，HTTP 请求直接失败且不会补发。规避：由客户端自行重试，未发送的操作先存在本地。实测 34 s 中断期间 100% 请求失败，恢复后无积压、无补发 |
