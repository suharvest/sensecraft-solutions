...
| 压力下入库返回 409 | **已在 `fix/a2-concurrency` 修复，待 harvest-pi 复测确认。** 原因：批次号原来靠「读当天最大序号 + 1」，并发请求读到同一份已提交状态、算出同一个号，固定 5 次重试躲不开；修复改为从取号表原子自增，批次号格式不变。复测确认前的规避：按物料串行入库（同一时刻只发一个请求），客户端收到 409 后带退避重试。修复前 Raspberry Pi 5 实测：并发 1 错误率 0%，并发 5 为 77.4%，并发 10 及以上为 100% |
| 出库返回 429 | **已在 `fix/a2-concurrency` 修复，待 harvest-pi 复测确认。** 原因：`slowapi` 原来对 `/api/materials/stock-out` 按来源 IP 限流 60 次/分钟，同一 NAT 后面的终端共用这一份额度；修复改为按已认证身份（API key / 会话）计数，阈值由 `BUSINESS_RATE_LIMIT` 控制，默认 600 次/分钟。复测确认前的规避：单个出口 IP 的持续出库速率控制在 1 次/秒以下，繁忙点位单独出口 IP 或错峰发送 |
| 网络或服务中断期间请求丢失 | 原因：REST 层没有离线队列和写入缓冲——重连与退避只覆盖 MCP 语音 WebSocket，HTTP 请求直接失败且不会补发。**本轮不修，作为已知限制保留。** 规避二选一：一是网关侧保证网络可用性（有线优先、UPS 供电、服务与客户端同处一个局域网，别让中断跨广域网），把不可用窗口压到设备重启时间；二是客户端做写入队列——出入库先落本地，联通后按顺序补发并按批次号去重，这项排在二期。实测 34 s 中断期间 100% 请求失败，恢复后无积压、无补发 |
