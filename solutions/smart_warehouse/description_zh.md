...
- **并发对同一物料入库会返回 HTTP 409 —— 已在 `fix/a2-concurrency` 修复，待 harvest-pi 复测确认。** 上表的数字（并发 5 时 77.4%，并发 10 及以上 100%）来自修复前的镜像。根因：系统生成的 `batch_no` 走「读当天最大序号 + 1」，并发请求读到的是同一份已提交状态，因此算出同一个号，固定 5 次重试躲不开。修复把取号换成 `batch_no_sequences` 表上的原子自增，批次号格式 `YYYYMMDD-NNN` 与老数据不变。**复测确认之前，仍按未修复对待：入库按物料串行，客户端收到 409 带退避重试。**
- **出库接口按来源 IP 限流 60 次/分钟 —— 已在 `fix/a2-concurrency` 修复，待 harvest-pi 复测确认。** 上表的 95–98% 429 来自修复前的镜像。修复把限流键改成「已认证身份（API key / 会话）优先，取不到再退回来源 IP」，阈值由 `BUSINESS_RATE_LIMIT` 环境变量控制，默认 600 次/分钟（10 次/秒）；注册、找回密码、设备校验那几个防枚举限速仍按 IP 计数不变。**复测确认之前，仍按未修复对待：单个出口 IP 的持续出库速率控制在 1 次/秒以下。**
