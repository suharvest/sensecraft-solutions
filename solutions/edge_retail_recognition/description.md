...
| reComputer J30 (Jetson Orin Nano, J3011, TensorRT) | TensorRT fp16 on the GPU, 5.88 ms p50, 99.27% agreement | TensorRT fp16 on the GPU, 5.06 ms p50, max 0.20pp retrieval gap vs fp32 | Also measured directly: full device-side loop run end to end (6726-frame checkout replay over 2.27 loops, zero dropped frames, all 84 events published) |
...
reComputer J40/J30 rows are measured on a reComputer unit itself — a
