...
而不是装一个包。装上去的是 `/userdata/f1-access` 下一组平铺的十二个纯标准库 Python
模块，以 root 跑在摄像头已有的人脸识别应用**旁边**：拉签名库、经回环 HTTP 让应用重载、
把应用的结果流映射成事件契约、驱动一路 sysfs GPIO。它不动 appmgr、不动 `/etc/init.d`、
不改应用的启动参数。这些步骤之所以是人工步骤，是因为它既不是安装包也不是 appmgr app，
...
万用表量过，没有接过 LED、继电器或锁，它在板上是哪一路也还没确认。阈值是识别应用自己的
默认值，不是标定结果（`calibration = pending`）。
...
## 步骤 4: 在摄像头上安装门禁守护进程 {#p1_install type=manual required=true config=devices/p1_recamera_pro.yaml}
...
适合能跑识别的盒子离门很远，或者一台盒子要管好几道门。识别跑在 J30/J40/R2000
