...
远程指令。2002 / 2002w 的开门指令经 MQTT 到网关侧继电器，broker 在开门路径上。

| 设备 | 作用 |
|---|---|
| 云端 / 本地服务器 | 人脸库服务、管理界面、MQTT broker |
| reCamera Pro 或标准版 reCamera | 摄像头、识别、活体、判定；Pro 与 HQ PoE 另输出 GPIO |
| reComputer R1000 或 XIAO ESP32-S3（仅 2002 / 2002w） | 在门口闭合触点 |
| 继电器模块 | COM/NO 干接点接到门控输入，按引脚电压选型（见步骤 5） |

*门锁、锁电源与门控本身是门控方的范围——不在这份 BOM 里。*

人脸库下发（下载、逐文件 SHA、验签、原子切换）已在 reCamera Pro 与标准版 reCamera 真机上
跑通；开门时间与拒绝次数见方案页"实测效果"。

**注意。** 这不是经过认证的安防或人身安全系统。人脸嵌入权重仅限非商用（见方案页许可说明）。
随包阈值需在现场用正负样本标定。
