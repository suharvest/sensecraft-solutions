...
万用表量过，没有接过 LED、继电器或门控。它在板上是哪一路已经确认（设备树 pinmux：
扩展口 UART4 M0 TXD 脚改配成 GPIO，属于 3.3 V 这一类），但空闲/驱动电平与可提供电流
还没实测。阈值是识别应用自己的默认值；还没有针对实测的识别/拒绝样本对做过标定。
...
2. **继电器模块，用它自己的供电。** 确认每个脉冲触点响一次。模块要与引脚匹配：
   ...
   UART 或 CAN 脚是 3.3 V。出厂默认的 `gpio130`（GPIO4_A2）经设备树 pinmux
   核实，是 UART4 M0 TXD 脚改配而来——属于 3.3 V 这一类，不是那两路 12–21 V
   原生输出之一——但实际电压与可提供电流仍需用万用表确认。默认继电器：Grove -
   Relay（SKU 103020005），SPST-NO，机械（非固态）触点，官方文档写明 3.3–5 V
   触发。门控输入是常闭型、需要 `NC` 端子的话换成 Grove - SPDT Relay(30A)
   （SKU 103020012）——厂商没有文档化它的 3.3 V 触发可靠性，要单独给它接 5 V
   供电，或者上板前先实测。固态继电器在这里不能用：它不是机械干接点，且直流
   负载一旦导通就关不掉。
