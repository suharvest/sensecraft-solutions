...
经过认证的安防产品；除标准版 reCamera 的拉库链路外，门控链路都没有在硬件上验证过。

## 实测到什么程度

**这不是经过认证的安防或人身安全系统，而且它从未在硬件上运行过。** 上游项目停在纯软件
里程碑：一台 macOS 开发机上的软件闭环，用假执行器、内存 MQTT broker 与假识别器。
没有摄像头、没有继电器、没有锁、没有真实 broker、没有真实人脸模型进过这条链路。
