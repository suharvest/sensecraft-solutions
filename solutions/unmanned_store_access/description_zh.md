...
**开门时间：第一帧回放帧交给应用，到 GPIO 引脚被写成触发电平。** 在设备上跑
「f1-access」 0.1.1 应用本身实测，覆盖取帧、检测、活体、比对、判定与写引脚。
p50，括号内为 p95，每档 12 次——n=12 时 p95 一列按上界读。
...
24 次里引脚 24 次被写成触发电平。条件：1280x720 帧按 12.5 fps 回放，活体开启，
「min_face_px」 40、「match_threshold」 0.40；探测者是一段素材视频的回放帧、
不是真人。写引脚之后 1500 ms 的触点保持不计入。未接继电器与锁，因此这些数不含
任何机械响应。10 人库与 1000 人库的 p50 实测相差 11 ms。来源：
unmanned-store-access 仓库
「evaluation/runs/2026-09-08-f1-0.1.1-validation/results.md」。

**拒绝：100 次里引脚一次都没有被写成触发电平。** 同一台设备、同一个应用，
每行 20 次。其中 40 次是未注册人，60 次是把屏幕举到镜头前。
...
未注册人那两行的库里只有合成向量，素材里那位不在库中。三个屏幕行的模板由攻击
素材本身建成，因此库里的脸与屏幕上的脸是同一个人。静止屏幕那一行用的是定住的
单帧屏幕画面，打印照片没有测。
