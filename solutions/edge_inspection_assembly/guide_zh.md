...
**重要说明。** 随包模型训练在 DeepPCB（裸板铜箔缺陷数据集）上，作用是把整条链路证通；
真实工位需要用你自己的图像重训。三个已知弱点要提前规划：期望件 ROI 是画面坐标，
摄像头一动模板就失效；零件倾斜或标定物与被测面不等距会让测量带偏；
所有摄像头共享一份 Modbus 寄存器，逐路结果只能从 MQTT 取。
...
**重要说明。** 随包模型训练在 DeepPCB（裸板缺陷数据集）上，真实工位请用自己的图像重训。
三个弱点同样适用——画面坐标 ROI、标定平面敏感、多路共享一份 Modbus 寄存器。
...
（fp32 CPU 参考 0.9876）；推理 p50 30.9 ms、p95 34.5 ms。同一模型的 fp16
...
**注意.** 本预设不开缺件比对与尺寸测量：这两项都要按工位标 ROI，通用表单
