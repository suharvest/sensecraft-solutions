...
| 指标 | 数值 | 条件 | 来源 |
|---|---|---|---|
| 检测 mAP50 | **0.9876** | DeepPCB6 val 205 图 / 1158 框 / 6 类；YOLOX-Tiny 640²，TensorRT fp16；冻结阈值 0.35 下 P 0.9284 / R 0.9741，FP 87 / FN 30，205 帧整帧漏检 0 | 本项目 M4 实测，2026-09-05，设备 Orin NX 16GB 工程参考套件（JetPack 6.2 / TRT 10.3） |
...
| reComputer R2000（Hailo-8）吞吐、时延与精度 | **硬件 106.75 FPS，全链路 43.92 FPS，mAP50 0.9858** | 硬件推理 854 帧 / 8 秒（「hailortcli run」）。精度：205 张 val 图 mAP50 0.9858，对比 CPU golden 差 -0.0018。应用层推理 94.02 FPS（P50 10.64 ms）。含判定、Modbus 与 MQTT 的全链路 43.92 FPS。10 fps 产线节拍下端到端时延 P50 11.89 ms / P99 16.08 ms | 同款 Hailo-8 平台实测参考值，reComputer 整机复测后更新，2026-09-06 |
| reCamera Pro（RV1126B）精度与时延 | **mAP50 0.9870，mAP50-95 0.8000，推理 P50 30.9 ms** | 同一份 205 张 DeepPCB6 val，YOLOX-Tiny 640²，RKNN INT8（rknn-toolkit2 2.3.2，64 张校准图，逐通道，「normal」）。同一批图上 fp32 CPU 参考：mAP50 0.9876、mAP50-95 0.8213。冻结阈值 0.35 下这一版报 P 0.9299 / R 0.9741，与 CPU 参考相同。P95 34.5 ms，205 次背靠背调用，相机自带应用已停止。同一模型的 fp16 版本：mAP50 0.9875、mAP50-95 0.8221、P50 110.3 ms | 本项目 reCamera Pro 实测，2026-09-08，reCamera Pro（librknnrt 2.3.2） |
| 半自动标注，box IoU | **均值 0.6896**，IoU ≥ 0.5 占 90.7%（1050 / 1158） | SAM2.1 Hiera-Small，仅给框提示，DeepPCB6 val 205 图 / 1158 框；IoU 是 SAM2 mask 的外接框与人工画的 GT 框的比对，跑在一台 GB10 GPU 工作站上，同机有另一个训练任务占着 GPU | 「edge-inspection-assembly」 标注工具实测，2026-09-05。不是本 demo 的检测精度——标注工具的代理指标，见下方一节 |
| 半自动标注，单框耗时 | **34.4 ms/框**（单图均值 194.5 ms） | 同上一行的运行与条件；比 50 张校准轮的 117 ms/图慢，是同机训练任务抢 GPU 导致的，不是模型变了 | 同一次标注工具实测 |

精度数字来自 DeepPCB，它比真实装配场景容易——人造 PCB 缺陷边界清晰。
上线前请用自己的板子重训。这是参考设计，不是经过认证的检测产品。
...
0.35 下这一版与 CPU 参考在这批图上的精确率与召回率相同。同一模型的 fp16 版本一并
