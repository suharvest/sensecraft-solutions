...
| reCamera Pro（RV1126B）精度与时延 | **mAP50 0.9870，mAP50-95 0.8000，推理 P50 30.9 ms** | 同一份 205 张 DeepPCB6 val，YOLOX-Tiny 640²，RKNN INT8（rknn-toolkit2 2.3.2，64 张校准图，逐通道，「normal」）。同一批图上 fp32 CPU 参考：mAP50 0.9876、mAP50-95 0.8213。冻结阈值 0.35 下这一版报的 P 0.9299 / R 0.9741 与 CPU 参考总数相同，但漏掉的 30 个框不是同一批；64 张校准图取自同一份验证集，因此 INT8 这一列偏乐观、偏多少没有量过。P95 34.5 ms，205 次背靠背调用，相机自带应用已停止。同一模型的 fp16 版本：mAP50 0.9875、mAP50-95 0.8221、P50 110.3 ms | 本项目 reCamera Pro 实测，2026-09-08，reCamera Pro（librknnrt 2.3.2） |
...
同一批。两条限制：INT8 的 64 张校准图取自同一份验证集，因此 INT8 这一列偏乐观、
偏多少没有量过；数字来自在设备上回放验证集图片，不是把相机对着板子拍。同一模型的 fp16 版本一并
