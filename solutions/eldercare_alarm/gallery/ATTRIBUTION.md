...
streams by local ffmpeg with `-c copy` for the local replay. The dataset itself
is not vendored in either repository — only these panel screenshots, which are
frame captures of that replay, are committed and distributed here. Redistribution
of these frames follows the GMDCSA-24 dataset repository's MIT license
(<https://github.com/ekramalam/GMDCSA24-A-Dataset-for-Human-Fall-Detection-in-Videos>);
cite the GMDCSA-24 dataset and paper as requested by its authors. The source
clips are 640 x 480, which is why the 4:3 letterbox bars are visible in the
configuration screenshot. Live state comes from a local mosquitto plus
