...
| 现象 | 处理 |
|------|------|
| 部署连不上相机 | 相机必须通过 USB-C（`192.168.42.1`）或网络可达，并使用 `recamera` 用户的 SSH 密码 |
| 应用启动后立刻退出 | 模型加载失败——确认部署已把 `fastdepth_224_bf16.cvimodel` 放到 `/userdata/local/models/` |
| 重启后应用没有自己回来 | 预期行为——init 脚本以停用状态安装（`K92`）。启动由控制台负责，见第 2 步 |
...
| 现象 | 处理 |
|------|------|
| 无法启用应用，且顶部显示 **Node-RED 模式**横幅 | 先切回 Console 模式——Node-RED 模式下画廊应用会被停止和禁用 |
| 刚启用应用又丢了摄像头 | Node-RED 被手动停掉后被 supervisor 脚本拉了回来，复活的 Node-RED 在和应用抢摄像头。切到 Console 模式 |
...
如果整张图都很平，先看场景再怀疑模型——大面积无纹理表面确实不足以让它工作。
